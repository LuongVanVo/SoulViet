...
public class PostShareEventService : IShareEventService
{
    private readonly IConnectionMultiplexer _redis;
    private readonly SseConnectionManager _sseManager;
    private readonly ILogger<PostShareEventService> _logger;
    private readonly ConcurrentDictionary<Guid, int> _subscriptionCounts = new();
    private readonly object _lock = new();

    public PostShareEventService(
        IConnectionMultiplexer redis,
        SseConnectionManager sseManager,
        ILogger<PostShareEventService> logger)
    {
        _redis = redis;
    ...
    public async Task PublishShareChangedAsync(Guid postId, object sharePayload, CancellationToken cancellationToken = default)
    {
        var channel = $"shares:{postId}";
        var json = JsonSerializer.Serialize(sharePayload, new JsonSerializerOptions
        {
        ...
        try
        {
            ...
            _logger.LogDebug("[SSE] Published share event to channel {Channel}", channel);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[SSE] Error publishing share event for post {PostId}", postId);
            throw;
        }
    }
    ...
    private void OnRedisMessageReceived(RedisChannel channel, RedisValue message)
    {
        ...
        _ = _sseManager.BroadcastAsync(postId, "share", message.ToString(), id).ContinueWith(t =>
        {
            if (t.IsFaulted)
                _logger.LogError(t.Exception, "[SSE] Error broadcasting share event to SSE clients for post {PostId}", postId);
